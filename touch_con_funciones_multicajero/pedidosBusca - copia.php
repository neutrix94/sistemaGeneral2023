<?php
    ...
    if($tipo == 1){
      //sacamos el año actual
        ...
        $sql="SELECT
              ...
              c.nombre,
              p.total,
              total-(SELECT IF(SUM(monto) IS NULL, 0, SUM(monto)) FROM ec_pedido_pagos WHERE id_pedido = p.id_pedido),
      ...
      //implementación de Oscar 31.05.2018 para que solo muestre regstros del año actual
        $sql.=" AND p.fecha_alta LIKE '%".$a_fech."%'";

        if($clave!= ''|| $clave!=null){
        //separamos la búsqueda para buscar coincidencias por palabra
          $txt=explode(" ",$clave);
        //buscamos coincidencias del cliente
          $sql.=" AND(";
          for($i=0;$i<sizeof($txt);$i++){
            if($txt[$i]!=''||$txt[$i]!=null){
              ...
              $sql.=" c.nombre LIKE '%".$txt[$i]."%'";
            }
          }//fin de for i

          $sql.=")";
        }//FIN DE IF CLAVE ESTA VACÍA 
            
        
        if($folio != '')
            $sql.=" AND IF(p.folio_nv IS NULL, p.folio_pedido, p.folio_nv) LIKE '%$folio%'";
        
        if($id_pedido != '')
            $sql.=" AND p.id_pedido = $id_pedido";
        
      //ORDENAMIENTO Y LÍMITE DE RESULTADOS
        $sql.=" ORDER BY p.id_pedido DESC  LIMIT 30";// modificación de Oscar 30.05.2018 para mostar má de 30 pedidos LIMIT 30      
    ...
    if($tipo == 3)
    {
        ...
        $sql="SELECT
              (p.total-(SELECT IF(SUM(monto) IS NULL, 0, SUM(monto)) FROM ec_pedido_pagos WHERE id_pedido=p.id_pedido)),
              p.id_pedido
              FROM ec_pedidos p
...
/*************Pagos y apartados************/
        ...
        for($i=0;$i<$numdatos;$i++){
          /*implementación Oscar 09.08.2018*/
            ...
            //buscamos cantidad total de productos de Pedro y externos
              $sql="SELECT
                      ROUND((aux.interno)/aux.total,3),
                      ROUND((aux.externo)/aux.total,3)
                    FROM(
                      SELECT 
  ...
  //die("contador:".$cont);
            for($j=0;$j<=$cont;$j++){
              if($dato1[$i] == 'NO'){  
                if($j==0||$j==1&&$pago_externo>0){
                  ...
                  hora = CURTIME(),";
                  if($j==0){
                    $cs.="monto=".round($dato5[$i]*$pago_interno).",";
                  }else{
                    $cs.="monto=".round($dato5[$i]*$pago_externo).",";
                  }
                //monto = '{$_GET["mon{$ix}"]}',
                $cs.="referencia = '',
                id_moneda = '1',
        ...
        $sql="UPDATE ec_pedidos p SET pagado=IF((p.total-(SELECT IF(SUM(monto) IS NULL, 0, SUM(monto)) FROM ec_pedido_pagos WHERE id_pedido=p.id_pedido)) <= 0, 1, 0) WHERE p.id_pedido=$id_pedido";
    ...
    //calculamos el final del LIMIT
      if($pag==1){
        ...
        $sql="SELECT
              ...
              WHERE p.id_pedido > 0
              AND p.id_sucursal = $user_sucursal
			       AND(p.id_estatus <> 7 OR(SELECT COUNT(1) FROM ec_devolucion WHERE id_pedido = p.id_pedido) <= 0)";
