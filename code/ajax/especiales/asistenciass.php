...
/*implementacion Oscar 2023 para registrar errores de asistencia*/
  function insert_error( $store_id, $user_id, $type, $login, $error_msg, $link ){
    $sql = "INSERT INTO ec_registros_nomina_erroneos (id_registro_erroneo, id_sucursal, id_usuario, tipo_asistencia, login_ingresado,
    mensaje_error, fecha_intento ) VALUES( NULL, {$store_id}, {$user_id}, '{$type}', '{$login}', '{$error_msg}', NOW() )";
    mysql_query( $sql ) or die( "Error al insertar mensaje de error : {$sql}" . mysql_error() );
  }
/*fin de cambio Oscar 2023*/
  ...
  //verificamos el password si es el caso
    if($verificar_pss==1){
      ...
      if(mysql_num_rows($eje)!=1){
        insert_error( $sucursal_id, $user_id, "actualiza_salidas", $pss, "Contraseña de encargado incorrecta!!!", $link );
  ...
  //extraemos el id del usuario
    ...
    //validamos que no haya una entrada menor al registro de salida
      ...
      //die($sql);
      if($r1[0]==0 && $r1[0]!=''){
        insert_error( $sucursal_id, $id_usuario, "actualiza_salidas", "N/A", "No se puede poner una hora mayor a la ultima entrada!!! verifique sus horas y vuelva a intentar", $link );
        die("No se puede poner una hora mayor a la ultima entrada!!! verifique sus horas y vuelva a intentar");
      }

    //validamos que no haya una entrada menor al registro de salida
      $sql="SELECT IF(hora_entrada>'$tmp[1]' OR (fecha=current_date() AND '$tmp[1]'>current_time()),0,1) FROM ec_registro_nomina WHERE id_registro_nomina=$tmp[0]";
      $eje=mysql_query($sql)or die("Error al validar hora de salida de registro de nomina contra la entrada!!!".mysql_error());
      $r1=mysql_fetch_row($eje);
      //die($sql);
      if($r1[0]==0){
        insert_error( $sucursal_id, $id_usuario, "actualiza_salidas", "N/A", "No se puede poner una hora de salida menor a la entrada ni mayor a la hora actual!!! verifique sus horas y vuelva a intentar", $link );
        die("No se puede poner una hora de salida menor a la entrada ni mayor a la hora actual!!! verifique sus horas y vuelva a intentar");
      }
    ...
    //  die($sql);
      ...
      insert_error( $sucursal_id, $id_usuario, "actualiza_salidas", "N/A", "SALIDAS ACTUALIZADAS EXITOSAMENTE hora_salida={$tmp[1]} id_registro_nomina={$tmp[0]}", $link );
    }
    die('ok');
  }
...
//si no coincide regresamos error
    if($num <= 0){
      ...
      insert_error( $sucursal_id, $user_id, ( $tipo_registro == 1 ? "Entrada" : "Salida"), $llave, "Código inválido", $link );
       die( '<h5 style="text-align : center; color : red;font-size:800%;position:absolute:top:5%;">ERROR</h5>'.
            '<p align="center" style="position:absolute:top:10%;width:100%;color:white;font-size:600%;">'.
            '<b>Código inválido!!!</b></p>|error');
    }
//guardamos datos de consulta en variables
  ...
  //if($tipo_registro==1){
    ...
    /*2*/ r_n.hora_entrada,
    /*3*/ r_n.hora_salida,
    /*4*/  (SELECT CONCAT(ELT(WEEKDAY(r_n.fecha) + 1, 'Lunes', 'Martes', 'Miercoles', 'Jueves', 'Viernes', 'Sabado', 'Domingo')))
    ...
    if(mysql_num_rows($eje)>=1){
      echo '<p align="center" style="color:white;font-size:40px;position:absolute;top:100px;width:100%;"><b>Salidas pendientes de registrar</b></p>';
      echo '<div style="position:absolute;top:250px;width:50%;left:25%;background:white;height:300px;overflow:scroll;">';
      echo '<table width="100%;">';
        echo '<tr>';
          echo '<th style="background:rgba(225,0,0,.6); color:white;padding:8px;">Dia</th>';
    ...
    //total de salidas pendientes
      ...
      echo '<button id="btn_ok" style="position:absolute;top:160px;padding:20px;right:10%;font-size:40px;" onclick="guarda_cambios_salidas();">';//<!--onclick="location.reload();"-->
      echo '<b>Aceptar</b>';
      echo '</button>';
      insert_error( $sucursal_id, $id_empleado, ( $tipo_registro == 1 ? "Entrada" : "Salida"), $llave, "Emergente de salidas pendientes", $link );
      die('');
    } 
  //}
  ...
  //insertamos la entrada
    $sql="INSERT INTO ec_registro_nomina(fecha, hora_entrada, id_empleado, id_sucursal,sincronizar)
                                      VALUES('{$fcha}', '{$hora_reg}', {$id_empleado}, {$user_sucursal}, 1 )";
    mysql_query($sql) or die("Error en:\n$sql\n\nDescripcion:\n".mysql_error()); 
    insert_error( $sucursal_id, $id_empleado, ( $tipo_registro == 1 ? "Entrada" : "Salida"), $llave, "REGISTRO DE NOMINA INSERTADO EXITOSAMENTE : {$fcha} {$hora_reg}", $link );                                         
  ...
  //buscamos la ultima entrada pendiente de salida
    ...
    $eje=mysql_query($sql)or die("Error al consultar la ultima entrada registrada!!!".mysql_error());
    if(mysql_num_rows($eje)<=0){

      insert_error( $sucursal_id, $id_empleado, ( $tipo_registro == 1 ? "Entrada" : "Salida"), $llave, "No hay registros de entrada el dia de hoy para el usuario<br>{$empleado}", $link );
      die( '<div style="text-align : center;"><h5 style="color:red;font-size:700%;">Error</h5>' . 
        '<p align="center" style="position:absolute:top:0%;width:100%;color:white;font-size:500%;">'.
        '<b>No hay registros de entrada el dia de hoy para el usuario<br>'.$empleado.'!!!</b></p></div>|error');
    }
    $r=mysql_fetch_row($eje);
  //actualizamos las salidas
    ...
    insert_error( $sucursal_id, $id_empleado, ( $tipo_registro == 1 ? "Entrada" : "Salida"), $llave, "HORA DE SALIDA ACTUALIZADA A {$hora_reg}", $link );
    mysql_query($sql) or die("Error en:\n$sql\n\nDescripcion:\n".mysql_error());                                          
