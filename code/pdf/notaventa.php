<?php
	...
	$pdf->Image("fondonotas.png" , 0 , 0, 21.6, 27.9);
	...
	$sql="SELECT 
	      folio_pedido AS folio,
	      CONCAT(u.nombre, ' ', u.apellido_paterno, ' ', u.apellido_materno) AS hechapor,
	      DATE_FORMAT(p.fecha_alta, '%Y-%m-%d') as fecha,
	      c.nombre as cliente,
	      CONCAT(d.calle, ' #', d.no_ext, ' Col. ', d.colonia, ' CP ', d.cp) AS dir,
	      p.id_direccion AS tipdir,
	      ...
	      JOIN ec_clientes_domentrega d ON p.id_direccion = d.id_clientes_de
	      WHERE id_pedido=$id";
	...
	$pdf->celpos(16.5, 4.1, 10, $ftam, $fecha,0,"L");
	...
	//$pdf->celpos(3.5, 8.2, 6.7, $ftam+2, $hechapor,0,"C");	 
	if($tipdir != '-1')
	{
	...
	$sql="SELECT
	      ...
	      CONCAT('$', FORMAT(d.monto, 2)) AS monto
	...
	$sql="SELECT
	      ...
	      CONCAT('$', FORMAT(monto, 2)) AS precio
	      FROM ec_pedidos_otros
	      WHERE id_pedido=$id";
