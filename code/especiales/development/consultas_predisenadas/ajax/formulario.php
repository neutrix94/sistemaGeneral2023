...
/*implementación Oscar 2021 para ejecutar consultas con MYSQLI*/
	...
	if($id_h!=0){
		$sql="SELECT 
			...
			WHERE id_herramienta_desarrollo = {$id_h}";
...
/*implementacion Oscar 2021 para poner el tipo de consulta*/
	$combo_tipos = "<select id=\"query_type\" class=\"form-control\" style=\"width:70%; display:inline;\">"
			. "<option value=\"Horizontal\" " . ($r[13]== 'Horizontal' ? 'selected' :null) . ">Horizontal -- </option>"
			. "<option value=\"Vertical\"" . ($r[13]== 'Vertical' ? 'selected' :null) . ">Vertical I</option>"
		. "</select>";

	echo '<button type="button" title="Cerrar" onclick="document.getElementById(\'emergente\').style.display=\'none\';" class="btn btn-danger btn_cerrar"><i class="icon-cancel"></i></button>';
	echo '<form>';
		echo '<table class="table" style="background : white !important;">';//tabla_formulario
				echo '<tr>';
					...
					echo '<td width="80%" class="titulo"> Tipo : ' . $combo_tipos . '</td>';
				echo '</tr>';
				echo '<tr>';
					echo '<td class="titulo">Título:</td>';
					echo '<td><input id="titulo" class="form-control" value="' . $r[1] .'"></td>';
				echo '</tr>';
				echo '<tr>';
				...
				echo '</tr>';
				echo '<tr>';
					echo '<td class="titulo">API</td>';
					echo '<td><input type="text" id="api_route" class="form-control" value="' . $r[14] . '"></td>';
				echo '</tr>';
			/*botones*/
				echo '<tr>';
