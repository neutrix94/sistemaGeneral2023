...
/*0-$CARACTER_REEMPLAZAR|1-campo_comparacion|2-tipo_elemento|3-consulta_combo/Formato fecha|4-onchange|5-id_elemento_html*/
		...
		for($i=1;$i<sizeof($r);$i++){
			...
			//si es combo
				if( $arr[2] == 'combo' ){
					echo '<select class="seleccion" style="width:100%;" id="'.$arr[5].'" datosDB="'.$arr[3].'" onchange="'.$arr[4].'" caracter_cambio="'.$arr[0].'" campo_filtrar="'.$arr[1].'">';
						...
						echo '<option class="opciones1 form-control" value="0">--Ver Todo--</option>';
			...
			//si es fecha
					...
					echo '<input type="text" style="width:100%;" class="form-control" id="'.$arr[5].'" value="" datosDB="'.$arr[3].'" onclick="'.$arr[4].'" caracter_cambio="'.$arr[0].'" campo_filtrar="'.$arr[1].'">';
				}
			//si es fecha y hora
				else if ( $arr[2] == 'datetime' ){
					echo '<input type="datetime-local" class="fech form-control" id="'.$arr[5].'" datosDB="'.$arr[3].'" onclick="'.$arr[4].'" caracter_cambio="'.$arr[0].'" campo_filtrar="'.$arr[1].'">';
				}
				echo '</div>';
