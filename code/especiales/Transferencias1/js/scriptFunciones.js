...
/*Implementación de Oscar 23.10.2018 para habilitar */
...
/*implementación Oscar 17.08.2018 para validar paquetes y agregar susu productos*/
...
/*implementación de Oscar 14.06.2018 para editar celdas de estacionalidad máxima e inventario*/
...
/*implemntación de Oscar 01.08.2018 para desplazarse entre celdas editables*/
...
//función que modifica el inventario
...
/*Modificación para exportación de Transferencias Oscar 28.05.2018*/
	var ventana_abierta;
	...
	var dats='Id producto,Orden de lista,Clave Proveedor,Nombre,Inv Origen,Inv Destino,Estacionalidad,Pedido,Ubicacion Matriz,Ubicación Destino\n';
	//extraemo los datos de la tabla
	...
	/*fin del cambio*/
  	...
  	//enviamos datos por ajax
  		$.ajax({
  			...
  			data:{id:id,sOr:sucOrigen,sDes:sucDestino,aOrigen:almOrigen,aDestino:almDestino,c:parseInt(nFilasIniciales),cant:cantidad,fl:flag},
  			success: function(datos){
  				//alert(datos);
				var resultados=datos.split("|");//SEPARAMOS RESPUESTA DE AJAX
				if(resultados[0]!='ok'){
					alert("Error!!!\n"+datos);
					return false;
				}
...
/*funcion implementada por Oscar para subir, bajar el número de presentaciones del producto*/
...
//funcion que valida teclas de desplazamiento
...
//FUNCIÓN QUE MUESTRA RESULTADOS DE NUEVA FILA
function buscar(e){
	var tecla=(document.all) ? e.keyCode : e.which;
		//alert(tecla);
...
//funcion que altera a la transferencia
function actualizar(){
...
/**/
	$.ajax({
		...
		cache:false,
		data:{transfer_id:idTr,adic:info,pres:pres,idPro:idPro,ped:ped,tot:tot},
		success: function(datos){
			if(datos=='ok'){
...
//funcion que resalta fila y enfoca cantidad
...
//mandamos llamar función de enfoque cuando la pagina carga
function enfocar(){
...
//funcion que guarda a la transferencia en bd
	function confirmar(){
		var nota="";
		var revisa_datos=0;
		if(nota_confirmada==0){
			var msg='<p align="right"><button style="padding:10px;background:red;font-size:20px;color:white;" ';
				...
				msg+='Observacion/notas de la Transferencia';
...
//alert('validos: '+cuenta+"\nNo validos"+noCuenta);
		...
		//enviamos datos por ajax
			$.ajax({
				...
				data:{adic:info,pres:pres,idPro:idPro,ped:ped,tot:tot,nota_transfer:nota,racionar:raciona},//enviamos arreglos
				cache:'false',
				dataType:'html',
				success:function(datos){
					if(datos=='ok'){
