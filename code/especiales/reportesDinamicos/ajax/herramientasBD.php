...
/*implementación Oscar 2021 para ejecutar consultas con MYSQLI*/
	...
	//id de herramienta
		if($id=='(Automático)'){
			$sql="INSERT INTO sys_reportes_dinamicos SET id_reporte_dinamico = null,";
		}else{
			$sql="UPDATE sys_reportes_dinamicos SET";
		}
	//titulo
		$tit=$_POST['titulo'];
		$sql.=" titulo='".$tit."',";
	...
	//si es actualización completa la instrucción
		if($id!="(Automático)"){
			$sql.=" WHERE id_reporte_dinamico = ".$id;
		}
		$eje=$link->query($sql)or die("Error al insertar/actualizar reporte dinámico!!!".$link->error."\n".$sql);
