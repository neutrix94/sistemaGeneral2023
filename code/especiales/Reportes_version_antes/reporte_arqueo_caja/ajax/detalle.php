...
?>
		...
		<center>
			...
			<?php
				$sql="SELECT 
						a.no_afiliacion,
						SUM(IF(scd.id_sesion_caja_detalle is null,0,scd.monto_validacion))
					FROM ec_sesion_caja_detalle scd 
					LEFT JOIN ec_afiliaciones a ON a.id_afiliacion=scd.id_afiliacion
					LEFT JOIN ec_sesion_caja sc ON sc.id_sesion_caja=scd.id_corte_caja
					WHERE sc.fecha='$fechaLim1' AND sc.verificado=1";
					//IF($fecha=-1,sc.fecha='$fechaLim1',(sc.fecha BETWEEN '$fechaLim1' AND '$fechaLim2') )
			
				$eje=mysql_query($sql)or die("Error al consultar los pagos con tarjeta!!!<br>".mysql_error().'<br>'.$sql);
				while ($r=mysql_fetch_row($eje)){
					echo '<tr>';
						echo '<td align="right">'.$r[0].':</td>';
						echo '<td align="right">'.$r[1].'</td>';
					echo '</tr>';
				}
			?>
		</table>
