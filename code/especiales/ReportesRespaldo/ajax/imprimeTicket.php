...
//insertamos las tarjetas
	for($i=0;$i<sizeof($arr_tarjetas)-1;$i++){//insertamos las tarjetas
		...
		$sql="INSERT INTO ec_sesion_caja_detalle VALUES(null,$id_sesion_cajero,$arr[0],'-1',$arr[1],'',now(),1)";
		$eje=mysql_query($sql);
		if(!$eje){
			$error=mysql_error();
...
//insertamos lo cheques
	for($i=0;$i<sizeof($arr_cheques)-1;$i++){//insertamos las tarjetas
		...
		$sql="INSERT INTO ec_sesion_caja_detalle VALUES(null,$id_sesion_cajero,'-1',$arr[0],$arr[1],'$arr[2]',now(),1)";
		$eje=mysql_query($sql);
		if(!$eje){
			$error=mysql_error();
			mysql_query("ROLLBACK");//cancelamos la transacción
...
/*insertamos el efectivo*/
	$sql="INSERT INTO ec_sesion_caja_detalle VALUES(null,$id_sesion_cajero,'-1','-1',$monto_en_efectivo,'efectivo',now(),1)";
	$eje=mysql_query($sql);
	if(!$eje){
		$error=mysql_error();
		mysql_query("ROLLBACK");//cancelamos la transacción
...
//cerramos la sesión de la caja
	$sql="UPDATE ec_sesion_caja SET hora_fin=now(),total_monto_ventas=$total_pagos_corte WHERE id_sesion_caja=$id_sesion_cajero";
	$eje=mysql_query($sql);
	if(!$eje){
		$error=mysql_error();
		mysql_query("ROLLBACK");//cancelamos la transacción
