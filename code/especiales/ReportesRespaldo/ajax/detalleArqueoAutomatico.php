...
/*sacamos el total de cobros totales*/
	$sql="SELECT SUM(IF(cc.id_cajero_cobro IS NULL,0,cc.monto)) FROM ec_cajero_cobros cc WHERE cc.id_cajero=$user_id AND fecha='$fcha_corte'";
	//die($sql);
...
//sacamos los cobros en efectivo
	$sql="SELECT SUM(IF(cc.id_cajero_cobro IS NULL,0,cc.monto)) FROM ec_cajero_cobros cc WHERE cc.id_cajero=$user_id AND fecha='$fcha_corte' AND id_banco=-1 AND id_afiliacion=-1";
	$eje=mysql_query($sql)or die("Error al sacar el total de pagos en efectivo!!!\n".mysql_error());
	$efectivo_ax=mysql_fetch_row($eje);
	$efectivo=$efectivo_ax[0];
...
/*Tarjetas*/
		...
		<tr>
			<td align="right" colspan="1">
				<b>Pagos con cheque/Transferencia:</b>
			</td>
		</tr>
		<?php
/*Cheques*/
		...
		?>
				...
				<tr>
					...
					<td align="right"><b><?php echo $efectivo;?></b></td>
				</tr>
			</table>
		...
		<br>
			...
			?>
				...
				<tr>
					...
					<td align="right"><b><?php echo round($efectivo-$gastoTotal,2);?></b></td>
				</tr>
			</table>
	...
	<!--Termina tabla detalle de Descuentos-->
		...
		<input type="button" value="Cerrar caja e imprimir" class="boton" onclick="generaTicket();">
		</div>
	</center>