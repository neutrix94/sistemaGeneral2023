...
/*2. Implementacion Oscar 15.08.2018 para filtrar productos externos*/
...
/*fin de cambio 15.08.2018*/
	...
	$otroFiltro="";
//3. Consulta para transferencia existente
if( isset( $idTransfer ) ){
...
//asignamos valores
	...
	/*$sql = "SELECT
				...
				p.orden_lista AS ordenLista,
				p.nombre AS Nombre,
				SUM( tp.cantidad ) AS CantidadPresentacion,
				1 AS Presentacion,
		...
		$sql = "SELECT
					...
					aux.clave,
					aux.Nombre,
					aux.CantidadPresentacion,
					aux.cantidadSurtir,
					aux.Presentacion,
					aux.maximo,
					aux.idEstProd,
					aux.ubicacion_almacen,
					aux.ubicacion_almacen_sucursal,
					aux.observaciones,
					aux.productProviderDetail,
			 		SUM(IF(m.id_almacen=$al_origen,IF(md.cantidad IS NULL, 0, md.cantidad*tm.afecta),0)) AS InvOr,
					SUM(IF(m.id_almacen=$al_destino,IF(md.cantidad IS NULL, 0, md.cantidad*tm.afecta),0)) AS InvDes
				FROM(
					SELECT
						...
						REPLACE(p.clave,',','*') AS clave,
						p.nombre AS Nombre,
						SUM( IF( tp.id_producto_or = pp.id_producto, tp.cantidad, 0 ) ) AS CantidadPresentacion,
						SUM( tp.cantidad ) AS cantidadSurtir,
						1 AS Presentacion,
						ep.maximo,
						ep.id_estacionalidad_producto AS idEstProd,
						p.ubicacion_almacen,
						sucP.ubicacion_almacen_sucursal,
						p.observaciones,
						CONCAT(SUM( tp.cantidad ), '||',
							GROUP_CONCAT( 
								CONCAT( pp.id_proveedor, '',
									prv.nombre_comercial, '',
							...
							(
								SELECT IF( pp1.id_proveedor IS NULL, 
										'',
										CONCAT( '||', 
											GROUP_CONCAT( 
												CONCAT( pp1.id_proveedor, '',
													prv1.nombre_comercial, '',
													pp1.id_proveedor_producto, '',
													ipp1.inventario, '',
													pp1.clave_proveedor, '',
													pp1.presentacion_caja, '',
													pp1.piezas_presentacion_cluces, '',
													0, '',
													0, '',
													0, '',
													0, ''
								...
								FROM ec_proveedor_producto pp1
								LEFT JOIN ec_proveedor prv1
								ON prv1.id_proveedor = pp1.id_proveedor
								LEFT JOIN ec_inventario_proveedor_producto ipp1
								ON ipp1.id_proveedor_producto = pp1.id_proveedor_producto
								AND ipp1.id_almacen = '{$al_origen}'
								WHERE pp1.id_producto = p.id_productos
								AND pp1.id_proveedor_producto 
								NOT IN( SELECT 
											id_proveedor_producto 
										FROM ec_transferencia_productos 
										WHERE id_producto_or IN( pp.id_producto )
				...
				LEFT JOIN sys_sucursales_producto sucP 
				ON sucP.id_producto=tp.id_producto_or 
				AND sucP.id_sucursal = '{$destino}'
				LEFT JOIN ec_estacionalidad_producto ep 
				ON tp.id_producto_or=ep.id_producto  
				LEFT JOIN ec_proveedor_producto pp
				ON pp.id_proveedor_producto = tp.id_proveedor_producto
				LEFT JOIN ec_proveedor prv 
				ON prv.id_proveedor = pp.id_proveedor
				LEFT JOIN ec_inventario_proveedor_producto ipp
				ON ipp.id_proveedor_producto = pp.id_proveedor_producto
				AND ipp.id_almacen = '{$al_origen}'
				WHERE id_transferencia = '{$idTransfer}'
...
//4. consulta para nueva transferencia
...
$sql="SELECT 
			...
			IF(au.stock_bajo=1 AND au.racionar_transferencias_productos=1 AND au.racion_tomada = 0,
				(au.racion_3/au.Presentacion),
				IF(au.stock_bajo=0 ,
					au.CantidadPresentacion ,
					0
				)
			)AS CantidadPresentacion,
			/*au.CantidadPresentacion AS CantidadPresentacion,*/
			au.cantidadSurtir,
			au.Equivalencia,
			au.Diferencia,
			au.idEstProd,
			...
			au.stock_bajo,
			IF( au.racion_tomada = 1, 0, au.racion_3 ) As racion_3,/*oscar 2023 para que no vuelva a tomar la racion*/
			/*au.racion_3,*/
			au.observaciones,
			au.minimo_surtir
	FROM(
		...
		/*Fin de cambio Oscar 26.02.2019*/
			...
			sucP.ubicacion_almacen_sucursal,
			sucP.stock_bajo,
			sucP.racion_tomada,/*oscar 2023 para que no vuelva a tomar la racion*/
			sucP.racion_3,
...
/*Fin de cambio Oscar 19.03.2019*/
						...
						/*IF(pres.id_producto_presentacion IS NULL,1,pres.cantidad)*/1 AS cantPres,
						/*IF(pres.id_producto_presentacion IS NULL,'PIEZA',pres.nombre)*/'PIEZA' AS nomPres,
				/*Implementación Oscar 26.02.2019 para incluir el alfanumérico*/
						aux1.clave,
				/*Fin de cambio Oscar 26.02.2019*/
						(SELECT
							...
							AND trans.id_estado BETWEEN '2' AND '8'
				...
				/*Fin de cambio Oscar 24.03.2019*/
					...
					/*Fin de cambio Oscar 26.02.2019*/
						...
						/*Implementación Oscar 08.05.2019 para sacar las ventas (por medio de los movimientos de almacen)*/
							SUM(IF(m.id_movimiento_almacen IS NOT NULL AND alm.es_almacen=1 AND m.id_almacen!=1 
								AND tm.id_tipo_movimiento=2 AND alm.es_externo=0 AND m.fecha like '%$act_year%',md.cantidad,0)) AS ventasTodasSucs,
							p.observaciones
						/*fin de cambio Oscar 08.05.2019*/
							FROM ec_productos p
		...
		/*Implementación Oscar 19.03.2019 para la transferencia complementaria*/
					...
					AND(trc.id_estado BETWEEN 1 AND 8 )
					AND trc.id_sucursal_destino IN($destino)
					AND trc.fecha LIKE '%{$act_year}%'
					GROUP BY aux2.ID
		/*Fin de Cambio Oscar 19.03.2019*/
				)aux
...
/*5. Implementacion Oscar 27.02.2019 para mostrar datos del encabezado de Transferencias*/
	$sql="SELECT
