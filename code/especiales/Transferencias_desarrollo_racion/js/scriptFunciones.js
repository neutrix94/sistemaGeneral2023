		...
		if(nota_confirmada==0){
			...
			<div class="row text-center">
				...
				<h2 class="text-center">Título</h2>
					<textarea id="titulo_transfer" style="width:80%;height:60px;">`;
					if( $('#tipo').val() == 1 ){
						msg += `(URGENTE)`;
					}
					msg += `</textarea><br>
					...
					<option value="${transfer_type}">Normal</option>
					<option value="1">Urgente</option>
					</select><br />
					<h2>Observacion/notas</h2>
					<br><textarea id="obs_transfer" style="width:80%;height:200px;"></textarea><br>	
				</div>
				...
				<div class="row">
						...
						<div class="col-8">
							<br>
							<button 
								onclick="nota_confirmada=1;confirmar();" 
		...
		for(var i=1;i<=nFilasFinales;i++){
			if(!document.getElementById('fila'+i) ){//COMPROBAMOS SI LA FILA EXISTE
					...
					if(comprueba==0 || comprueba==""){
						...
						|| parseInt( $( "#racion_" + i ).html().trim() ) > 0 ){//oscar 2023
						
						cuenta++;//incrementamos contador
						final_request_data += (final_request_data != '' && i > 0 ?  '|~|' : '' );
						final_request_data += $('#1_'+i).html()+'~';
						final_request_data += $( '#13_' + i ).html().trim();
						if( $( '#racion_' + i ) .html().trim() != '0' ){//indicador de racio
							final_request_data += "~" + 'es_racionado';
						}
					}
			}
		}//fin de for i
...
/*Cambio Oscar 2023*/
		...
		$( '#consumablesList tr' ).each( function ( index ){
			if( $( '#consumables_0_' + index ).val() > 0 ){
				//alert( 1 );
				consumables_array += ( consumables_array == "" ? "" : "|~|" );
				consumables_array += $( '#consumables_1_' + index ).attr( 'value' ) + "~";
				consumables_array += $( this ).attr( "value" ) + "~";
				consumables_array += $( '#consumables_0_' + index ).val();
			}else{
				//alert( 2 );
				//alert( $( "#consumables_1_" + index ).prop( 'checked' ) );
				if( $( "#consumables_1_" + index ).prop( 'checked' ) == false ){
					valid = index;
					return false;
				}
			}
		...
		//alert( valid );
		if( valid != 'ok' ){
			alert( "Si no vas a pedir algo debes de marcar el check \"No\"" );
			$( "#consumables_0_" + valid ).focus();
			return false;
		}
		if( consumables_array == "" ){
			if( ! confirm( "Continuar sin pedir consumibles?" ) ){
				return false;
			}
		}
		/*alert( consumables_array );
		return false;*/
...
function get_consumables(){
	var url = "ajax/getConsumables.php?fl_type=withoutPriority";
	var resp = ajaxR( url );
	return resp;
}
//llamadas asincronas
...
/*implementacion Oscar 2023 para exportar el archivo de la racion*/
	
	function download_ration_debug(){
		var dats = "";//dats = "ID PRODUCTO,ORDEN LISTA,PRODUCTO,SUCURSAL,INVENTARIO SUCURSAL,ESTACIONALIDAD,FALTANTE,";
		//dats += "VENTAS SUCURSAL,% VENTAS SUCURSAL,RACION SUCURSAL,TOTAL VENTAS SUCURSALES,TOTAL INVENTARIO SUCURSALES,INVENTARIO MATRIZ";
		$( "#ration_debug_table tr" ).each( function( index ){
			dats += ( dats == "" ? "" : "\n" );
