...
$_GET['counter'] );*/
	...
	function calculateProductProvider( $product_id, $quantity, $link, $counter = null, $permission = 0 ){
		...
		$date =" ";
		$last_date = "IF( rec.fecha_recepcion IS NOT NULL, MAX( rec.fecha_recepcion ), '' ) AS last_date";
		$order_by = " ORDER BY MAX( rec.fecha_recepcion ) ASC";
		if( $row[0] == 2 ){
			$last_date = "IF( rec.fecha_recepcion IS NOT NULL, MAX( rec.fecha_recepcion ), '' ) AS last_date";
			$order_by = " ORDER BY pp.prioridad_surtimiento ASC, MAX( rec.fecha_recepcion ) DESC";
		}else if( $row[0] == 3 ){//implementracion Oscar 2023 regla ult ent prim sal s/prioridad
			$last_date = "IF( rec.fecha_recepcion IS NOT NULL, MAX( rec.fecha_recepcion ), '' ) AS last_date";
			$order_by = " ORDER BY pp.fecha_ultima_compra DESC";
		}//fin de cambio regla ult ent prim sal s/prioridad Oscar 2023
