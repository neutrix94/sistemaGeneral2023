...
//5. Inserta el detalle de la transferencia
	...
	foreach ( $details as $key => $detail ) {
		...
		if( $es_racionado == 1 ){//actualiza el indicador de racion
			$sql = "UPDATE sys_sucursales_producto 
						SET racion_tomada = 1 
					WHERE id_sucursal = {$adic[2]} 
					AND id_producto = {$product_id}";
			$eje_update = mysql_query($sql);//EJECUTA CONSULTA
				if( ! $eje_update ){
					$error=mysql_error();
					mysql_query("ROLLBACK");
					die("Error al actualizar el indicador de producto racionado!!!\n\n".$sql."\n\n".$error);
				}

		}
	}
/*Implementacion Oscar 2023 para insertar el detalle de consumibles*/
	$consumables_array = ( isset( $_POST['consumables_array'] ) ? $_POST['consumables_array'] : $_GET['consumables_array'] );
	$consumables_array = explode( "|~|", $consumables_array );
	foreach ( $consumables_array as $key => $consumables ) {
		if( $consumables != '' && $consumables != null ){
			$consumable = explode( "~", $consumables );
			$counter ++;
			$sql = "INSERT INTO ec_transferencia_productos( /*1*/id_transferencia, /*2*/id_producto_or, 
				...
				/*7*/referencia_resolucion, /*8*/cantidad_cajas, /*9*/cantidad_paquetes, /*10*/cantidad_piezas, 
				/*11*/id_proveedor_producto, /*12*/numero_consecutivo )
			VALUES(/*1*/'{$nuevo}',/*2*/'{$consumable[0]}',/*3*/'-1',/*4*/'{$consumable[2]}',/*5*/'{$consumable[2]}',
				/*6*/'{$consumable[0]}',/*7*/'{$consumable[2]}',/*8*/'0',/*9*/'0', 
				/*10*/'{$consumable[2]}', /*11*/'{$consumable[1]}',/*12*/'{$counter}' )";
			$eje=mysql_query($sql);//EJECUTA CONSULTA
			if(!$eje){
				$error=mysql_error();
				mysql_query("ROLLBACK");
				die("Error al insertar detalle / consumibles de la Transferencia!!!\n\n".$sql."\n\n".$error);
			}
		}
	}
