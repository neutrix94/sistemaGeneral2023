	...
	//recorremos la tabla
		for( var i = 0; i <= tope; i++ ){
			if( document.getElementById( 'fila_' + i ) ){//si existe la fila
				...
				//extraemos datos
					datos+=$("#1_"+i).html()+"~";//extraemos el id de producto[0]
				//piezas
					var tmp=0;
					...
					datos+=$("#7_"+i).html().trim()+"~";//extraemos precio por pieza[2]
					datos+=$("#8_"+i).html().trim()+"~";//extraemos el monto por producto[3]
					datos+=$("#4_"+i).html().trim()+"~";//extraemos presentación por caja[4]
				}//fin de else

				datos+=$("#11_"+i).html().trim()+"~";//extraemos el descuento[5]
				if( $("#13_"+i).val() == 0 ){
					alert( "Hay productos sin proveedor, verifique y vuelva a intentar" );
					$("#13_"+i).focus();
					return false;
				}
				datos +=$("#13_"+i).val();//proveedor_producto[6]
				datos += '~' + $("#0_"+i).html().trim();//id recepcion detalle[7]
				datos += '~' + $("#-2_"+i).html().trim();//validado / no validado[8]
				datos += '~' + $("#6_"+i).html().trim();//piezas sueltas recibidas[9]
				datos += '~' + $("#5_"+i).html().trim();//cajas recibidas[10]

				datos += '~' + $("#6_6_"+i).html().trim();//total validando[11]
				datos += '~' + $("#9_"+i).html().trim();//total piezas[12]
				datos += '~' + $("#4_3_"+i).html().trim();//total piezas validadas[13]
				datos += '~' + $("#4_1_"+i).html().trim();//cajas validadas[14]
				datos += '~' + $("#4_2_"+i).html().trim();//piezas sueltas validadas[15]
...
//finalización de la recepción
	function finish_reception( check_permission = null ){
		...
		$( '#table_body tr' ).each( function ( index ){
			count = parseInt( parseInt( index ) + 1 );
			var remission_pieces = parseInt( $( '#-5_' + count ).html().trim() );
			var total_pieces = parseInt( $( '#9_' + count ).html().trim() );
			if( remission_pieces != total_pieces ){
				is_correct = count;
				return false;
			}
		});
		if( is_correct != 'ok' ){
			var product_name = $( '#2_' + count ).html().trim();
			var content = `<h2>El producto <b class="text-warning">${product_name}</b> no coincide entre el total de piezas en remision 
				y el total de piezas, por lo tanto no se pude guardar y finalizar la remision\nVerifica y vuelve a intentar</h2>
				<div class="row">
					<div class="col-4"></div>
					<div class="col-4">
						<button
							class="btn btn-success"
							onclick="close_emergent();document.getElementById( '-5_${count}' ).click();setTimeout( function(){document.getElementById( 'entrada_temporal' ).focus();}, '500' );"
						>
	...
	function changeProductProvider( obj, product_id, count ){
		...
		//consulta los datos de proveedor producto
			...
			var subtotal=0,porcentaje_desc=0,subtotal_desc=0,total=0;
				...
				subtotal += parseInt( $( '#4_3_' + count ).html().trim() );
...
//guardar nueva remision
	...
	function getPendingOrdersDetail( product_id ){
		var url = "ajax/db.php?fl=getPendingOrdersDetail&provider_id=" + $( "#id_prov" ).val();
