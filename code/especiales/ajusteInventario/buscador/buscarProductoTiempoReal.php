<?php
	...
	//generamos consulta
		$sql="SELECT 
				CONCAT( p.nombre, '( MODELO : ', pp.clave_proveedor,' - Caja con ', pp.presentacion_caja ,' pzas)'), 
				p.id_productos,
...
/*Implementación Oscar 27.02.2019 para no mostrar resultados de prouctos maquilados ni que muestren paleta*/
		$sql.=" AND p.es_maquilado=0 AND p.muestra_paleta=0 GROUP BY pp.id_proveedor_producto";
...
//die($sql);
	...
	//de lo contrario;
	echo '<table class="table" width="100%" id="resulta">';//declaramos tabla de resultados
	echo'<tr><td></td></tr>';
			...
			//generamos opcion
				echo '<tr class="opcion" onclick="validaProducto('. $row[1] . ', ' . $row[2] . ');">
