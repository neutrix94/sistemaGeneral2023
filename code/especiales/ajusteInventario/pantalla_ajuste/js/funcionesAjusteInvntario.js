//declaramos variables globales
	...
	function reset_all_products(){
		...
		var url = "ajax/inventoryAdjustment.php?inventory_fl=reset_all_products&warehouse_id=" + current_warehouse;
		url += "&mannager_password=" + pass;
		var response = ajaxR( url ).split( '|' );
		if( response[0] != 'ok' ){
			alert( "Error : \n" + response );
			return false;
		}else{
			show_emergent( response[1], false, false );
		}
	}

	function product_count_reset(){
		var url = "ajax/inventoryAdjustment.php?inventory_fl=getOmmitedRows&warehouse_id=" + current_warehouse;
		var response = ajaxR( url );
		show_emergent( response, false, false );
	}
...
//funcion que guarda ajuste de Inventario
	...
	//mandamos valores por ajax
		$.ajax({
			...
			cache:'false',
			data:{quita:cR+resta,agrega:cS+suma,suc:sucursal, sin_movimientos : equals },
