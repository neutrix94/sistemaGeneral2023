<?php
	...
	*/
	...
	//
		...
		function getAfiliaciones( $user_id, $fecha_sesion, $hora_inicio_sesion, $hora_cierre_sesion ){
			$tarjetas_cajero='';
			$sql="SELECT 
					a.id_afiliacion,
					a.no_afiliacion 
			...
			//sumamos los pagos del cajero en caso de tener pagos
				...
				$tarjetas_cajero.='<tr class="is_card_row">';
					$tarjetas_cajero.='<td align="center">';
						$tarjetas_cajero.='<select id="tarjeta_'.$this->cards_counter .'" class="form-select" style="width:95%"><option value="'.$r[0].'">'.$r[1].'</option>';
					$tarjetas_cajero.='</td>';
					$tarjetas_cajero.='<td>';
						$tarjetas_cajero.='<input type="text" class="form-control text-end" id="t'.$this->cards_counter.'" value="'.$total.'" onkeyup="cambia_valor(this,\'ta'.$this->cards_counter.'\');" readonly>';
					$tarjetas_cajero.='</td>';
				$tarjetas_cajero.='</tr>';	
			}
