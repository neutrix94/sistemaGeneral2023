<?php
	...
	$archivo=fopen("ec_productos_presentaciones.csv","r");
	if(!$archivo){
...
//marcamos inicio de transaccion
	...
	for($i=2;$i<sizeof($aux);$i++){
		...
		//	WHERE id_productos=$rw[0]");/*,ubicacion='$rw[5]'*/
			$sql=utf8_encode("UPDATE ec_productos_presentaciones SET cantidad='$rw[1]' WHERE id_producto=$rw[0]");/*,ubicacion='$rw[5]'*/
		echo '<br>'.$sql;	
		$eje=mysql_query($sql);
		if(!$eje){
