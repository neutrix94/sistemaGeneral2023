<?php
	...
	$archivo=fopen("reporte_con_ubicacion.csv","r");
	if(!$archivo){
...
//marcamos inicio de transaccion
	...
	for($i=2;$i<sizeof($aux);$i++){
		...
		//	WHERE id_productos=$rw[0]");/*,ubicacion='$rw[5]'*/
			$sql=utf8_encode("UPDATE ec_productos SET ubicacion_almacen=REPLACE('$rw[1]', ' ', ' ,'),ultima_modificacion='2018-09-25 00:00:02' WHERE id_productos=$rw[0]");/*,ubicacion='$rw[5]'*/
		echo '<br>'.$sql;	
		$eje=mysql_query($sql);
		if(!$eje){
