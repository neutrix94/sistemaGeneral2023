...
//obtener productos con códigos de barras de decoración
	$sql = "SELECT 
			...
			AND id_categoria IN( SELECT id_categoria FROM ec_categoria WHERE nombre = 'Luces' )";
	$stm = $link->query( $sql ) or die( "Error al consultar las decoraciones por cambiar : {$link->error}");
	while ( $row = $stm->fetch_assoc() ) {
	/*	var_dump( $row );
	die('ok');*//*$row['piece_barcode_3'] == '' &&*/
		if( $row['pack_barcode_2'] == '' && $row['box_barcode_2'] == '' ){//$row['piece_barcode_3'] == ''
			$sql = "UPDATE ec_proveedor_producto 
						SET codigo_barras_pieza_1 = '', 
