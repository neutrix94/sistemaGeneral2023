<?php
//1. Incluye archivo %/concetMin.php%%
	include('../../../../conectMin.php');
	$fl=$_POST['fl'];

//2. Implementacion Oscar 23.10.2018 para activar productos de transferencia en la sucursal destino
	if($fl=='activa'){
...
//3. Implementacion de Oscar 28.05.2018 para generar archivo de exportacion CSV
	if($fl==1){
	//recibimos datos
		$info=$_POST['datos'];
...
//4. implementacion de Oscar 30.09.2019 para generar archivo de exportación CSV en orden de almacen matriz
	if($fl=='orden_almacen'){
	//recibimos datos
		$info=$_POST['datos'];
	//consultamos los datos iniciales de la tranasferencia
		...
		$sql="SELECT
				...
				ax.nombre,/*3*/
				SUM(
					IF(ma.id_movimiento_almacen IS NULL OR ma.id_almacen!=$alm_or,0,(md.cantidad*tm.afecta))
				) as invOrigen,/*4*/
				SUM(
					IF(ma.id_movimiento_almacen IS NULL OR ma.id_almacen!=$alm_des,0,(md.cantidad*tm.afecta))
				) as invDestino,/*5*/
				ax.maximo,/*6*/
				ax.cantidad,/*7*/
			...
			/*implementacion Oscar 2021 para ordenar por más de un almacen*/
			...
			AND s.id_sucursal={$suc_des}
			AND e.id_estacionalidad = s.id_estacionalidad
	...
	//creamos el nombre del archivo
		$nombre="Transferencia.csv";
		$info="Id producto,Orden de lista,Clave Proveedor,Nombre,Inv Origen,Inv Destino,Estacionalidad,Pedido,Ubicacion Matriz,Ubicación Destino,num cj, Observaciones del producto";
		for($i=0;$i<$tam;$i++){
			$r=mysql_fetch_row($eje);	
			$info.="\n";//salto de linea
			$info.=$r[0].",".$r[1].",".$r[2].",".$r[3].",".$r[4].",".$r[5].",".$r[6].",".$r[7].",".$r[8].",".$r[9].",".($i+1).",".$r[10];
		}
	//generamos descarga
		header('Content-Type: aplication/octect-stream');
		header('Content-Transfer-Encoding: Binary');
...
//5. Descarga de formato en limpio
	if($fl=="formato_limpio"){
	//creamos el nombre del archivo
		$nombre="formatoDeTransferencia.csv";
		$info='Id producto,Orden de lista,Clave Proveedor,Nombre,Inv Origen,Inv Destino,Estacionalidad,Pedido,Ubicacion Matriz,Ubicación Destino, Observaciones del Producto';
	//generamos descarga
		header('Content-Type: aplication/octect-stream');
		header('Content-Transfer-Encoding: Binary');
		header('Content-Disposition: attachment; filename="'.$nombre.'"');
...
//6. Verificacion password de encargado
	if($fl==2){
...
//7. Insercion de ajuste de inventario
	if($fl==3){
	...
	//insertamos la cabecera del movimiento
		...
		$sql="INSERT INTO ec_movimiento_almacen VALUES(NULL,$id_tipo,$user_id,$user_sucursal,NOW(),NOW(),'AJUSTE DESDE TRANSFERENCIA',-1,-1,'',-1,-1,$id_almacen,-1,0,'0000-00-00 00:00:00',NULL)";
		$eje=mysql_query($sql);
		if(!$eje){
			$error=mysql_error();
...
//8. Modificacion de maximo de estacionalidad
	if($fl==4){
