...
/*fin de cambio Oscar 02.10.2018*/
	...
	<!--Nombre del producto-->
		<td align="left" width="30.5%" title="<?php echo 'Clave: '.$row['clave'];?>" id="<?php echo '2_'.$c;?>" onclick="<?php echo 'resaltar('.$c.');';?>" <?php echo $resalta_rojo;?>><?php echo $row['Nombre'];?></td>
    ...
    <!--Inventario Almacén Destino-->
		<td align="right" width="9%" id="<?php echo '4_'.$c;?>" <?php echo $resalta_rojo;?> onclick="<?php echo 'resaltar('.$c.');';if($user_sucursal==$sDes){echo 'editaCelda(4,'.$c.');';}?>"><?php echo $row['InvDes'];?></td>
	<!--Estacionalidad máxima-->
		<td width="9%" align="right" id="<?php echo '5_'.$c;?>"  <?php echo $resalta_rojo;?> onclick="<?php if($user_sucursal==$sDes){echo 'editaCelda(5,'.$c.');';}?>"><?php echo $row['maximo'];?></td>
	<!--Cantidad por pedir-->
	...
	<!--Implementación Oscar 2021 para agregar la fila de productos ya solicitados en transferencias-->
		<?php
			$sql_pend = "SELECT
					IF(
				...
				WHERE trans_prd.id_producto_or = {$id}
				AND sp_1_1.id_producto={$id}
				AND trans.id_estado BETWEEN '2' AND '5'
				AND s_1_1.id_sucursal = '{$sDes}'";
				//die($sql);
			$prods_pend = mysql_query( $sql_pend )or die("Error al consultar transferencias pendientes : " 
				. mysql_error());
