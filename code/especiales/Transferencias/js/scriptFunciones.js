	...
	function confirmar(){
		...
		if(nota_confirmada==0){
			...
			/*implementación Oscar 2021 para título de transferencia*/
				msg += '<h2>Título de la Transferencia</h2>';
				msg+='<textarea id="titulo_transfer" style="width:80%;height:80px;"></textarea><br>';
		/*implementacion Oscar 2021 para cambiar transferencia a urgente*/
				if( $('#tipo').val() == 5 ){
					msg += '<h5> Libre / Urgente </h5>'
						+ '<select onchange="transfer_type_change(this);" style="padding : 8px;">'
							+ '<option value="5">Libre</option>'
							+ '<option value="1">Urgente</option>'
							+ '</select><br />';
				}
			/*fin de cambio Oscar 2021*/
				msg+='<h4>Observacion/notas de la Transferencia</h4>';
...
//alert('validos: '+cuenta+"\nNo validos"+noCuenta);
		...
		//enviamos datos por ajax
			$.ajax({
				...
				data:{adic:info,pres:pres,idPro:idPro,ped:ped,tot:tot,nota_transfer:nota,racionar:raciona,
				titulo : titulo_trans },//enviamos arreglos
				cache:'false',
				dataType:'html',
				success:function(datos){
					if(datos=='ok'){
