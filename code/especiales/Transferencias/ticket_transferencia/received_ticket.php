...
/*implementacion Oscar 19.08.2019 para crear el codigo de barras para la credencial del usuario*/
...
//extraemos detalle de productos en transferencia
	$id_transfer = explode( ',', $id_transfer );
	foreach ($id_transfer as $key => $transfer_id) {
		$sql="SELECT
				...
				/*2*/tp.total_piezas_recibidas as cantidad,
...
//consulta lo que esta en resolucion 
		$sql = "SELECT
		...
		$resolution_stm = mysql_query( $sql ) or die( "Error al consultar los productos en resolucion : " . mysql_error() );
		$product_resolutions_counter = mysql_num_rows( $resolutions_counter );
		$resolutions_array = array();
	//guarda respuesta en arreglo
		while ( $row = mysql_fetch_assoc( $resolution_stm ) ) {
			array_push( $resolutions_array, $row);
		}

//consulta lo que se va a regresar
		$sql = "SELECT
					CONCAT( '( ', p.orden_lista, ' ) ', p.nombre) AS product_name,
					btr.piezas_faltantes AS missing_pieces,
					btr.piezas_sobrantes AS excedent_pieces,
			...
			WHERE btr.id_bloque_transferencia_recepcion = {$reception_block_id}";
		$resolution_stm = mysql_query( $sql ) or die( "Error al consultar los productos en resolucion : " . mysql_error() );
		$product_missings_counter = mysql_num_rows( $resolutions_counter );
		$missings_array = array();
	//guarda respuesta en arreglo
		while ( $row = mysql_fetch_assoc( $resolution_stm ) ) {
			array_push( $missings_array, $missings_array);
		}
...
//Aqui me quede ( calculo paroximado de lineas de productos totales para darle tamaño al ticket linea 234 )
	$lines_calculation = $product_resolutions_counter * 15;
	$product_missings_counter
...
//encabezado
	...
	$ticket->Cell(66, 3, "( RECEPCION )", "" ,0, "C");
...
//die( $transfers );
	if($impresa==0){
		mysql_query("BEGIN");
		$sql="UPDATE ec_transferencias SET impresa=1 WHERE id_transferencia IN( $transfers )";
