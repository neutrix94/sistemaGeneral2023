...
//mostrar / ocultar vistas del menú
	...
	//
		...
		$( '#btn_finish_reception' ).css( 'display', ( view == '.validate_transfers' ? 'block' : 'none' ) );
...
//alert( txt ); return false;
	...
	//alert( 'here' );
		...
		if( just_validate_unique_code != null ){
			url += "&just_validate_unique_code=" + just_validate_unique_code;
		}
...
//alert( global_tmp_barcode );//( global_permission_box != null ? 1 : null )
	...
	function confirm_product_was_separated( type, unique_code, barcode, is_a_box, flag, pieces_quantity = 0 ){
		var pss = $( '#manager_password' ).val();
		if( pss.length <= 0 ){
			alert( "La contraseña del encargado no puede ir vacía!" );
			$( '#manager_password' ).focus();
...
//just_validate_unique_code = null
			//validateBarcode(  );
				...
				if( flag == 'incorrect_model' ){

				}else if( flag == 'incorrect_product' ){

				}else if( flag == 'unique_code_resolution' ){

				}
				validateBarcode( '#barcode_seeker', 'enter', null, ( pieces_quantity == 0 ? null : pieces_quantity ), is_a_box, unique_code, 0, flag );
				return false;
			}
			//url = "ajax/db.php?fl=insertResolutionUniqueCode&";
			//response = ajaxR(  );
		}else if( type == 2 ){
				...
				if( $( '#missing_row_5_' + index ).html().trim() == '' ) {
					if( permission == null ){
						...
						var aux = `<div class="row">
									<div class="col-2"></div>
									<div class="col-8">
										<h5>Aun hay productos penddientes de recibir, si vas a finalizar la transferncia 
...
//agregado por oscar para solucionar error resolucion
	function saveResolutionTmp(){
//inserta registro de faltantes en la tabla temporal
		var pending = '';
		var excedent = '';
		$( '#missing_and_excedent_counter_capture tr' ).each( function ( index ){
			$( this ).children( 'td' ).each( function( index_2 ){ 
				pending += ( pending == '' ? '' : '|~|' );
				pending += $( "#does_not_correspond_row_1_" + index ).html().trim();//id transferencia producto
			});
		});
//inserta registro de excedentes / no corresponde en la tabla temporal
...
//alert( 'here' );
		...
		if( ! validateReceptionIsComplete( permission ) ){
			return false;
		}else{//agregado por oscar para solucionar error resolucion
			if( ! saveResolutionTmp() ){
				return false;
			}
		}
