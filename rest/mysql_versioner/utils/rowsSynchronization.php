...
//hacer / obtener jsons de movimientos de almacen
		public function getSynchronizationRows( $system_store, $destinity_store, $limit ){
			$resp = array();
			$sql = "SELECT 
					...
					LIMIT {$limit}";
		//die( $sql );
			$stm = $this->link->query( $sql ) or die( "Error al consultar los datos de jsons de registros de sincronizacion: {$this->link->error} : {$sql}" );
			$movements_counter = 0;
			//forma arreglo
...
//actualizacion de registros de sincronizacion
		public function updateRowSynchronization( $rows, $petition_unique_folio, $status = null, $sum = false ){
			$sql = "";
			if( $status != null ){//actualiza status y folio unico de peticion
				$sql = "UPDATE sys_sincronizacion_registros 
