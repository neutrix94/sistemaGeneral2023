...
/*
* Endpoint: obtener_registros_sincronizacion_transferencias
* Path: /obtener_registros_sincronizacion_transferencias
* Método: POST
* Descripción: Recupera y envia los registros de sincronizacion de transferencias que no se han sincronizado
*/
$app->get('/obtener_registros_sincronizacion_transferencias', function (Request $request, Response $response){
...
//variables
  ...
  if( $system_store == -1 ){//valida que el origen no sea linea
    $SynchronizationManagmentLog->release_sinchronization_module( 'ec_transferencias', ( $LOGGER['id_sincronizacion'] ? $LOGGER['id_sincronizacion'] : false ) );//liberar el modulo de sincronizacion
    return json_encode( array( "response"=>"La sucursal es linea y no puede ser cliente." ) );
  }
  
/*Comprobacion de registros de sincronizacion de transferencias ( peticiones anteriores ) 2024*/
  $req['verification'] = $generalRowsVerification->getPendingRows( $system_store, -1, 'sys_sincronizacion_registros_transferencias', 
...
/*Fin de comprobacion de registros de sincronizacion de transferencias*/
  
  $req["log"] = $SynchronizationManagmentLog->insertPetitionLog( $system_store, -1, $store_prefix, $initial_time, 
    'REGISTROS DE SINCRONIZACION TRANSFERENCIAS', 'sys_sincronizacion_registros_transferencias', ( $LOGGER['id_sincronizacion'] ? $LOGGER['id_sincronizacion'] : false ) );//inserta request
  $req["rows"] = $rowsSynchronization->getSynchronizationRows( $system_store, -1, $rows_limit, 'sys_sincronizacion_registros_transferencias', $req["log"]["unique_folio"], ( $LOGGER['id_sincronizacion'] ? $LOGGER['id_sincronizacion'] : false ) );//consulta registros pendientes de sincronizar
  $post_data = json_encode($req, JSON_PRETTY_PRINT);//forma peticion
  $result_1 = $SynchronizationManagmentLog->sendPetition( "{$path}/rest/v1/inserta_registros_sincronizacion_transferencias", $post_data, ( $LOGGER['id_sincronizacion'] ? $LOGGER['id_sincronizacion'] : false ) );//envia petición
...
/*Procesa Respuesta de comprobacion*/
  if( $result->rows_validation->log_response != null && $result->rows_validation->log_response != '' ){
    ...
    $update_log = $generalRowsVerification->updateLogAndJsonsRows( $result->rows_validation->log_response, $result->rows_validation->rows_response, 'sys_sincronizacion_registros_transferencias', ( $LOGGER['id_sincronizacion'] ? $LOGGER['id_sincronizacion'] : false ) );
    if( $update_log != 'ok' ){
  ...
  if( $result->rows_validation->rows_download != null && $result->rows_validation->rows_download != '' ){
    ...
    if( $download->verification == true ){
      if( sizeof($petition_log) > 0 ){
        ...
        $verification_req['rows_response'] = $generalRowsVerification->RowsValidation( $validation_rows, 'sys_sincronizacion_registros_transferencias', ( $LOGGER['id_sincronizacion'] ? $LOGGER['id_sincronizacion'] : false ) );//realiza proceso de comprobacion
        $verification_req['table_name'] = "sys_sincronizacion_registros_transferencias";
...
/*Fin de Respuesta de Comprobacion*/
  ...
  if( $result->ok_rows != '' && $result->ok_rows != null ){//actualiza registros exitosos
    $local_response_log = $rowsSynchronization->updateRowSynchronization( $result->ok_rows, $req["log"]["unique_folio"], 'sys_sincronizacion_registros_transferencias', 3, false, ( $LOGGER['id_sincronizacion'] ? $LOGGER['id_sincronizacion'] : false ) );
  }
  if( $result->error_rows != '' && $result->error_rows != null ){//actualiza errores
    $local_response_log = $rowsSynchronization->updateRowSynchronization( $result->error_rows, $req["log"]["unique_folio"], 'sys_sincronizacion_registros_transferencias', 2, false, ( $LOGGER['id_sincronizacion'] ? $LOGGER['id_sincronizacion'] : false ) );
  }
...
//obtiene fecha y hora actual y actualiza registro de petición
  ...
  if( $result->rows_download != '' && $result->rows_download != null ){
    ...
    //obtiene fecha y hora actual y actualiza registro de petición
      ...
      $post_data_1 = json_encode(array( "log"=>$resp["log"], "ok_rows"=>$insert_rows["ok_rows"], "table"=>"sys_sincronizacion_registros_transferencias", "status"=>"error" ), JSON_PRETTY_PRINT);//forma peticion
    }else{
  ...
  //obtiene fecha y hora actual y actualiza registro de petición
      ...
      $resp["log"] = $SynchronizationManagmentLog->updatePetitionLog( $result->log_download->destinity_time, $result->log_download->response_time, $result->log_download->response_string, 
        $result->log_download->unique_folio, ( $LOGGER['id_sincronizacion'] ? $LOGGER['id_sincronizacion'] : false ) );
      $resp["log"]["type_update"] = "rowsSynchronization";
      $post_data_1 = json_encode(array( "log"=>$resp["log"], "ok_rows"=>$insert_rows["ok_rows"], "table"=>"sys_sincronizacion_registros_transferencias" ), JSON_PRETTY_PRINT);//forma peticion
    }
  }else{
  //obtiene fecha y hora actual y actualiza registro de petición
    $result->log_download->response_string = "No llegaron registros de sincronizacion de actualizacion de ventas de linea a local";
    $result->log_download->destinity_time = $SynchronizationManagmentLog->getCurrentTime( ( $LOGGER['id_sincronizacion'] ? $LOGGER['id_sincronizacion'] : false ) );
    $resp["log"] = $SynchronizationManagmentLog->updatePetitionLog( $result->log_download->destinity_time, $result->log_download->response_time, $result->log_download->response_string, 
      $result->log_download->unique_folio, ( $LOGGER['id_sincronizacion'] ? $LOGGER['id_sincronizacion'] : false ) );
    $resp["log"]["type_update"] = "rowsSynchronization";
...
//envia peticion para actualizar peticion de linea a local
  ...
  $post_data = json_encode(array( "log"=>$resp["log"], 
      ...
      "table"=>"sys_sincronizacion_registros_transferencias",
    ), JSON_PRETTY_PRINT);
  ...
  $SynchronizationManagmentLog->release_sinchronization_module( 'sys_sincronizacion_registros_transferencias', ( $LOGGER['id_sincronizacion'] ? $LOGGER['id_sincronizacion'] : false ) );//liberar el modulo de sincronizacion
