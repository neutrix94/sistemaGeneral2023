...
/*Comprobacion de registros de sincronizacion de ventas ( peticiones anteriores ) 2024*/
...
/*Fin de comprobacion de registros de sincronizacion de ventas*/
  ...
  /*Procesa Respuesta de comprobacion*/
      if( $result->rows_validation->log_response != null && $result->rows_validation->log_response != '' ){
        ...
        $update_log = $generalRowsVerification->updateLogAndJsonsRows( $result->rows_validation->log_response, $result->rows_validation->rows_response, 'sys_sincronizacion_registros_ventas', ( $LOGGER['id_sincronizacion'] ? $LOGGER['id_sincronizacion'] : false ) );
        if( $update_log != 'ok' ){
  ...
  if( $result->ok_rows != '' && $result->ok_rows != null ){//actualiza registros exitosos
    $local_response_log = $rowsSynchronization->updateRowSynchronization( $result->ok_rows, $req["log"]["unique_folio"], 'sys_sincronizacion_registros_ventas', 3, false, ( $LOGGER['id_sincronizacion'] ? $LOGGER['id_sincronizacion'] : false ) );
  }
  if( $result->error_rows != '' && $result->error_rows != null ){//actualiza errores
    $local_response_log = $rowsSynchronization->updateRowSynchronization( $result->error_rows, $req["log"]["unique_folio"], 'sys_sincronizacion_registros_ventas', 2, false, ( $LOGGER['id_sincronizacion'] ? $LOGGER['id_sincronizacion'] : false ) );
  }
...
//obtiene fecha y hora actual y actualiza registro de petición
  ...
  if( $result->rows_download != '' && $result->rows_download != null ){
    ...
    //obtiene fecha y hora actual y actualiza registro de petición
      ...
      $post_data_1 = json_encode(array( "log"=>$resp["log"], "ok_rows"=>$insert_rows["ok_rows"], "table"=>"sys_sincronizacion_registros_ventas", "status"=>"error" ), JSON_PRETTY_PRINT);//forma peticion
   }else{
  ...
  //obtiene fecha y hora actual y actualiza registro de petición
      ...
      $resp["log"] = $SynchronizationManagmentLog->updatePetitionLog( $result->log_download->destinity_time, $result->log_download->response_time, $result->log_download->response_string, 
        $result->log_download->unique_folio, ( $LOGGER['id_sincronizacion'] ? $LOGGER['id_sincronizacion'] : false ) );
      $resp["log"]["type_update"] = "rowsSynchronization";
      $post_data_1 = json_encode(array( "log"=>$resp["log"], "ok_rows"=>$insert_rows["ok_rows"], "table"=>"sys_sincronizacion_registros_ventas" ), JSON_PRETTY_PRINT);//forma peticion
    }
  }else{
  //obtiene fecha y hora actual y actualiza registro de petición
...
//envia peticion para actualizar peticion de linea a local
  ...
  $post_data = json_encode(array( "log"=>$resp["log"], 
      ...
      "table"=>"sys_sincronizacion_registros_ventas"
    ), JSON_PRETTY_PRINT);
  ...
  $SynchronizationManagmentLog->release_sinchronization_module( 'sys_sincronizacion_registros_ventas', ( $LOGGER['id_sincronizacion'] ? $LOGGER['id_sincronizacion'] : false ) );//liberar el modulo de sincronizacion
