...
/*
* Endpoint: obtener_registros_sincronizacion_mov_p_p
...
//verifica si el log esta habilitado
  ...
  if( $LOGGER ){
    $LOGGER = $Logger->insertLoggerRow( '', 'sys_sincronizacion_registros_movimientos_proveedor_producto', $system_store, -1 );//inserta el log de sincronizacion $LOGGER['id_sincronziacion']
  }
...
/*Comprobacion de registros de sincronizacion de movimientos proveedor producto ( peticiones anteriores ) 2024*/
  $req['verification'] = $generalRowsVerification->getPendingRows( $system_store, -1, 'sys_sincronizacion_registros_movimientos_proveedor_producto', 
...
/*Fin de comprobacion de registros de sincronizacion de movimientos proveedor producto*/
...
/*Procesa Respuesta de comprobacion*/
  if( $result->rows_validation->log_response != null && $result->rows_validation->log_response != '' ){
    ...
    $update_log = $generalRowsVerification->updateLogAndJsonsRows( $result->rows_validation->log_response, $result->rows_validation->rows_response, 'sys_sincronizacion_registros_movimientos_proveedor_producto', ( $LOGGER['id_sincronizacion'] ? $LOGGER['id_sincronizacion'] : false ) );
    if( $update_log != 'ok' ){
...
/*Fin de Respuesta de Comprobacion*/
  ...
  if( $result->ok_rows != '' && $result->ok_rows != null ){//actualiza registros exitosos
    $local_response_log = $rowsSynchronization->updateRowSynchronization( $result->ok_rows, $req["log"]["unique_folio"], 'sys_sincronizacion_registros_movimientos_proveedor_producto', 3, false, ( $LOGGER['id_sincronizacion'] ? $LOGGER['id_sincronizacion'] : false ) );
  }
  if( $result->error_rows != '' && $result->error_rows != null ){//actualiza errores
    $local_response_log = $rowsSynchronization->updateRowSynchronization( $result->error_rows, $req["log"]["unique_folio"], 'sys_sincronizacion_registros_movimientos_proveedor_producto', 2, false, ( $LOGGER['id_sincronizacion'] ? $LOGGER['id_sincronizacion'] : false ) );
  }
...
//obtiene fecha y hora actual y actualiza registro de petición
  ...
  //obtiene fecha y hora actual y actualiza registro de petición
    $result->log_download->response_string = "No llegaron registros de sincronizacion de actualizacion de movimientos proveedor producto de linea a local";
    $result->log_download->destinity_time = $SynchronizationManagmentLog->getCurrentTime( ( $LOGGER['id_sincronizacion'] ? $LOGGER['id_sincronizacion'] : false ) );
    $resp["log"] = $SynchronizationManagmentLog->updatePetitionLog( $result->log_download->destinity_time, $result->log_download->response_time, $result->log_download->response_string, 
      $result->log_download->unique_folio, ( $LOGGER['id_sincronizacion'] ? $LOGGER['id_sincronizacion'] : false ) );
    $resp["log"]["type_update"] = "rowsSynchronization";
