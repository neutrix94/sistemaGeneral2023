...
/*verfifica que no este sincronizando / marca sincronizando
  $check = $SynchronizationManagmentLog->block_sinchronization_module( 'ec_devolucion' );
  if( $check != 'ok' ){
...
//valida que el origen no sea linea
  ...
  //liberar el modulo de sincronizacion
    $SynchronizationManagmentLog->release_sinchronization_module( 'ec_devolucion' );
...
//ejecuta el procedure para generar los movimientos de almacen
  $setMovements = $returnsSynchronization->setNewSynchronizationReturns( $system_store, $system_store, $store_prefix, $movements_limit );
  if( $setMovements != 'ok' ){
  //liberar el modulo de sincronizacion
    $SynchronizationManagmentLog->release_sinchronization_module( 'ec_devolucion' );
...
//die( 'here' );
//return json_encode( $req["returns"] );
//Valida path
//if ( sizeof( $req["returns"] ) > 0 ) {
  //inserta request
    $req["log"] = $SynchronizationManagmentLog->insertPetitionLog( $system_store, -1, $store_prefix, $initial_time, 'DEVOLUCIONES' );
  //forma peticion
    $post_data = json_encode($req, JSON_PRETTY_PRINT);//
//return $post_data;
  //envia petición
    $result_1 = $SynchronizationManagmentLog->sendPetition( "{$path}/rest/v1/inserta_devoluciones", $post_data );
//return $result_1;
...
//liberar el modulo de sincronizacion
  $SynchronizationManagmentLog->release_sinchronization_module( 'ec_devolucion' );
  /*}else if( sizeof( $req["returns"] ) <= 0 ){
  //regresa respuesta vacía
    return json_encode( array( "response" => "No hay devoluciones por sincronizar" ) );
  }*/
...
//regresa respuesta
  return 'ok';
 //return json_encode( array( "response" => "Devoluciones ok!" ) );
});
