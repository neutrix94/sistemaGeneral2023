...
//variables
  ...
  $req["rows"] = $rowsSynchronization->getSynchronizationRows( $system_store, -1, $rows_limit, 'sys_sincronizacion_registros_transferencias' );//consulta registros pendientes de sincronizar
  $req["log"] = $SynchronizationManagmentLog->insertPetitionLog( $system_store, -1, $store_prefix, $initial_time, 'REGISTROS DE SINCRONIZACION', 'sys_sincronizacion_registros_transferencias' );//inserta request
  $post_data = json_encode($req, JSON_PRETTY_PRINT);//forma peticion
  $result_1 = $SynchronizationManagmentLog->sendPetition( "{$path}/rest/sincronizacion/inserta_registros_sincronizacion_transferencias", $post_data );//envia petición
  ...
  if( $result->ok_rows != '' && $result->ok_rows != null ){//actualiza registros exitosos
    $rowsSynchronization->updateRowSynchronization( $result->ok_rows, $req["log"]["unique_folio"], 'sys_sincronizacion_registros_transferencias', 3, false );
  }
  if( $result->error_rows != '' && $result->error_rows != null ){//actualiza erores
    $rowsSynchronization->updateRowSynchronization( $result->error_rows, $req["log"]["unique_folio"], 'sys_sincronizacion_registros_transferencias', 2, false );
  }
  ...
  if( $result->rows_download != '' && $result->rows_download != null ){
    ...
    if( $insert_rows["error"] != '' && $insert_rows["error"] != null  ){//inserta error si es el caso
      ...
      $post_data = json_encode(array( "log"=>$resp["log"], "ok_rows"=>$insert_rows["ok_rows"], "table"=>"sys_sincronizacion_registros_transferencias" ), JSON_PRETTY_PRINT);//forma peticion
