...
/*implementacion Oscar 2021 para no pedir autorización de descuento*/
	...
	$sql = "SELECT pedir_password_devolucion FROM ec_configuracion_sucursal WHERE id_sucursal = '{$user_sucursal}'";
	$eje_dev = mysql_query( $sql ) or die ( "Error al consultar configuración de devolucion: " . mysql_error() );
	$row_dev = mysql_fetch_row( $eje_dev );
	echo '<input type="hidden" id="pedir_password_devolucion" value="' . $row_dev[0] . '" />';
