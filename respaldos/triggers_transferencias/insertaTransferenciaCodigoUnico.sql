DROP TRIGGER IF EXISTS insertaTransferenciaCodigoUnico|
DELIMITER $$
CREATE TRIGGER insertaTransferenciaCodigoUnico
BEFORE INSERT ON ec_transferencia_codigos_unicos
FOR EACH ROW
BEGIN
...
/*folios unicos*/
	...
	/*consulta el siguiente id*/
	    ...
	    WHERE table_name = 'ec_transferencia_codigos_unicos'
	    AND table_schema = database();
	/*actualiza folio unico*/
        SET new.folio_unico = CONCAT( prefix, '_TCU_', row_id );
		...
		SELECT 
			...
			NOW(),
			'insertaTransferenciaCodigoUnico',
			1
		FROM sys_sucursales 
