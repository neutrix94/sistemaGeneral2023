DROP PROCEDURE IF EXISTS parametrosAgrupaMovimientosAlmacenProveedorProductoPorDiaCRON|
DELIMITER $$
CREATE PROCEDURE parametrosAgrupaMovimientosAlmacenProveedorProductoPorDiaCRON(IN tipo_agrupacion_movimientos INTEGER(1), IN minimo_dias INTEGER(11))
BEGIN
	DECLARE done INT DEFAULT FALSE;
	DECLARE fecha_tmp DATE;
	DECLARE fecha_base VARCHAR(10);
	DECLARE recorre CURSOR FOR
		SELECT DATE_FORMAT(fecha_registro,'%Y-%m-%d') 
...
-- Se abre el cursor
		...
		loop_recorre: LOOP  	
			...
			IF done THEN
				LEAVE loop_recorre;
			END IF;
			CALL agrupaMovimientosProveedorProductoCRON(2,fecha_tmp);
		END LOOP;
-- cerramos el cursor
	CLOSE recorre;   
