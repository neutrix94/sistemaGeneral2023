DROP TRIGGER IF EXISTS after_insert_almacen_producto|
DELIMITER $$
CREATE TRIGGER after_insert_almacen_producto
AFTER INSERT ON ec_almacen_producto
FOR EACH ROW
BEGIN
    IF new.id_almacen = '1' and new.id_producto is not null THEN
        INSERT INTO ec_sync_magento(tipo,id_registro,estatus,detalle)
        
        select distinct 'Producto' tipo, almacen.id_producto id_registro, '1' estatus, 'insert' detalle
