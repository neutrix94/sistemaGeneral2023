...
BEGIN
    ...
    THEN
    	INSERT INTO ec_sincronizacion_registros VALUES(null,id_suc,IF(id_suc=-1,new.id_sucursal,-1),'ec_gastos',new.id_gastos,2,3,
        CONCAT("UPDATE ec_gastos SET ",
                ...
                "sincronizar=0 WHERE id_equivalente='",new.id_gastos,"' AND id_sucursal='",new.id_sucursal,"'"
        ),
        0,1,CONCAT('Se modificó un gasto por concepto de ',(SELECT nombre FROM ec_conceptos_gastos WHERE id_concepto=new.id_concepto)),now(),0,0,'id_gastos');
    END IF;
