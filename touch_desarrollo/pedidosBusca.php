...
/*************Pagos y apartados************/
    if($tipo == 4){
        ...
        /*implementación de Oscar 2023 para enlazar el pago a la sesion de caja*/
          $sql="SELECT 
            IF(multicajero=1,0,
                (SELECT 
                    id_cajero 
  ...
  //die("contador:".$cont);
        ...
        /*implementacion Oscar 28.06.2019*/
            $cs.=", id_cajero = {$id_cajero}, id_sesion_caja = {$id_sesion_caja}";//modificacion Oscar 2023 para enlazar el pago a la sesion de caja
        /*Fin de cambio Oscar 28.06.2019*/
                }
  ...
  //contamos el número de pedidos del año en curso
    ...
    //calculamos el final del LIMIT
      if($pag==1){
        ...
        $sql="SELECT
              ...
              p.fecha_alta,
              p.total,
              IF( p.venta_validada = 1, 
                  CONCAT( '<i class=\'icon-ok-circle\' style=\'color : green;\'></i>' ),
                  CONCAT( '<i class=\'icon-cancel-circled\' style=\'color : red;\'></i>' )
              )
              FROM ec_pedidos p
              JOIN ec_clientes c ON p.id_cliente = c.id_cliente
              WHERE p.id_pedido > 0
    ...
    /*implementación de Oscar para buscador 31.08.2018*/
      ...
      /*Implementación Oscar para sacar pedidos del año en curso 31.05.2018*/
        $sql.=" AND p.fecha_alta LIKE '%".$a_fech."%'";
      /*Fin de cambio*/
        $sql.=" ORDER BY p.id_pedido DESC";//modificación Oscar 30.05.2018 para mostrar má de 30 pedidos           
        $sql .= ( $folio == '' ? ' LIMIT 30' : '');
        $res=mysql_query($sql/*,$local*/) or die("Error en:\n$sql\n\nDescripcion:\n".mysql_error());      
    ...
    if( $tipo == 7 ){
      $sql = "SELECT
                ...
                IF(pvu.id_pedido_validacion IS NULL, 0, COUNT( pvu.id_pedido_validacion ) ) AS details_validated
              FROM ec_pedidos p
              ...
              WHERE p.id_pedido = {$id_pedido}";
      $stm = mysql_query( $sql ) or die( "Error al consultar si la venta esta validada parcialmente" );
      $row = mysql_fetch_assoc( $stm );
      if( ( $row['was_validated'] == 0 && $row['details_validated'] > 0 ) ){
        die( '<center><h5 style="font-size : 150%;" >La venta esta validada parcialmente y no se puede devolver desde esta pantalla, 
          Ve a la validación al apartado de edicion de nota de venta para modificarla.<h5>
          <br><br>
          <button type="button" onclick="close_emergent();" style="padding : 15px; color : white; background-color : green;
          font-size : 150%;">
            Aceptar
          </button></center>' );
      }
