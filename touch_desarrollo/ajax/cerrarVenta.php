...
/*Modificacion Oscar 2023/10/11 para no enlzar directamente el cajero a la venta cuando es unicajero*/
	...
	error_log("VALIDANDO COMPARACIÓN DESCUENTO, TOTAL Y SUBTOTAL");
	$qGetPedidoBack="SELECT id_pedido,folio_nv, fecha_alta, total, subtotal, descuento FROM ec_pedidos_back WHERE id_pedido = '{$id_pedido}'";

	error_log($qGetPedidoBack);

	$resultGetPedidoBack = mysql_query($qGetPedidoBack);
	...
	if( abs($totalEsperado - $totalPedido) > $epsilon ){
		...
		/**********************Insertamos los datos reales******************/
			...
			//Inserta la cabecera del pedido
				...
				error_log("touch_desarrollo/ajax/cerrarVenta.php");
				$res=mysql_query($sql);
