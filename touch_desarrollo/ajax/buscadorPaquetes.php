...
/************************************************************************busqueda de paquetes********************************************************************/
	...
	//filtro de paquetes activos
		$sql.=" WHERE pa.activo=1 AND pa.id_sucursal_creacion = {$sucursal_id}";
		if($texto!='muestra_todos'){
		...
		echo 'ok|<table class="t_p_1">';
			...
			//foramos fila
				echo '<tr id="fila_'.$c.'">';
					...
					echo '<td align="center" width="10%"><button type="button" class="btn btn-primary" onclick="modificar(1,'.$c.','.$r[0].')"><i class="icon-eye"></i></button></td>';
					echo '<td align="center" width="10%"><button type="button" class="btn btn-warning" onclick="modificar(2,'.$c.','.$r[0].')"><i class="icon-pencil"></i></button></td>';
					echo '<td align="center" width="10%"><button type="button" class="btn btn-secondary" onclick="abre_config_paqt('. $r[0] . ')"><i class="icon-pencil"></i></button></td>';
					echo '<td align="center" width="10%"><button class="btn btn-info" onclick="crea_transfer(' . $r[0] . ',' . $r[4] . ');" style=""><i class="icon-paper-plane-1"></i></button></td>';
					echo '<td align="center" width="10%"><button type="button" class="btn btn-danger" onclick="modificar(3,'.$c.','.$r[0].')"><i class="icon-eye"></i></button></td>';
				echo '</tr>';
			}
			echo '</table>';
			die("");
...
/****************************************************************buscador de productos para el detalle de paquetes***************************************************************/
	if($fl==2){
		$sql="SELECT p.id_productos,p.nombre
				...
				ON p.id_productos=sp.id_producto AND sp.id_sucursal=$user_sucursal
				LEFT JOIN ec_proveedor_producto pp
				ON pp.id_producto = p.id_productos
				WHERE p.habilitado=1
				AND sp.estado_suc=1
				AND (p.orden_lista='{$texto}'
					OR p.id_productos='{$texto}' 
					OR pp.codigo_barras_pieza_1 = '{$texto}'
	...
	//formamos tabla
		if( mysql_num_rows( $eje ) <= 0 ){
			die( 'ok|<b style="font-size : 150% !important;">Sin coincidencias, si estas escaneando un producto asegurate de escanear la pieza ( NO cajas NI paquetes )</b>' );
		}
	...
	<tr id="detalle_<?php echo $cont;?>">
		...
		<td>
			<input 
				...
				class="caja_editable" 
				onkeyup="document.getElementById('b_d_1_<?php echo $cont;?>').disabled=false;"
			>
		</td>
		<td align="center">
			<button 
...
/*******************************************************insertar nuevo paquete y su detalle***********************************************************************/
	...
	/*modificado por Oscar 2023 - se agrega sucural de creacion para que solo se habilite en esta sucursal */
		$sql="INSERT INTO ec_paquetes ( id_paquete, nombre, imagen, descripcion, activo, trans_generada, id_sucursal_creacion, ultima_actualizacion, sincronizar ) 
			VALUES(null,'$nom',null,'',$status,0, {$sucursal_id}, now(),1)";/*Se implementa el 1 el día 03.07.2018 Oscar*/
...
//die( "array : " . $arr );
		...
		for($i=0;$i<sizeof($arreglo1);$i++){
			...
			$sql="INSERT INTO ec_paquete_detalle ( id_paquete_detalle, id_paquete, id_producto, 
				cantidad_producto, sincronizar )/*modificacion Oscar 2023*/
					SELECT
					null,
					{$nvo_id},
					{$arreglo_tmp[0]},
