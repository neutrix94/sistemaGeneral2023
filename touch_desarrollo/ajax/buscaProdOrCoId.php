...
/*Fin de cambio Oscar 04.03.2019*/
  ...
  //consultamos si el producto es de Daniel
    $sql="SELECT 
            sp.es_externo 
          FROM ec_productos p 
          ...
          CONCAT(p.nombre,'<br><b>',
            IF(s.mostrar_ubicacion=1,
                IF($user_sucursal=1,
                  ...
                  IF(( SELECT 
                        CONCAT( numero_ubicacion_desde, altura_desde ) 
                    ...
                    ( SELECT 
                        CONCAT( 'Ubicación: ', numero_ubicacion_desde, altura_desde, '  ' ) 
                      FROM ec_sucursal_producto_ubicacion_almacen 
                      WHERE id_sucursal = {$user_sucursal}
                      AND es_principal = 1
                      AND id_producto = p.id_productos
...
//die( "<textarea>{$sql}</textarea>" );          
    ...
    $sql="SELECT
        ...
        /*Fin de cambio*/
          (SELECT prod.es_ultimas_piezas FROM ec_productos prod WHERE prod.orden_lista = '{$val}' LIMIT 1 ) AS is_last_pieces
          FROM ec_productos p
          JOIN sys_sucursales s ON s.id_sucursal=$user_sucursal
          JOIN sys_sucursales_producto sp ON sp.id_producto=p.id_productos
          LEFT JOIN ec_precios_detalle pd ON /**/$lista_prec/**/ AND pd.id_producto = p.id_productos AND $can2 >= de_valor AND $can2 <= a_valor
          WHERE ( sp.id_producto='$val' OR p.orden_lista = '{$val}' )
...
//die($sql);          
    ...
    if(mysql_num_rows($res) > 0){
      ...
      //capturamos resultados
        ...
        die("exito|$row[0]|$row[1]|$row[2]|$row[3]|$row[4]|$row[5]|$row[6]|$row[7]|$row[8]|NO|$row[9]|$row[10]|$row[11]");
    }
