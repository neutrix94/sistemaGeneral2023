...
function CargaGrid(NomId)
{
	...
	for(var j=0;j<Tds.length;j++)
	{
		...
		if(tipos[j] == 'libre')		
		{			
			cab[j]='<input type="button" value="" style="width:'+(largo)+'px; height:20px;" class="buttonHeader" id="H'+NomId+j+'" onkeydown="TeclasEspeciales(event, this, \''+NomId+'_'+(j+1)+'_NaN\');"/>';
	...
	{	
		...
		caux+='onchange="'+funcam[j]+'" verSumatoria="'+verSum[j]+'" combolargo="'+combolargo[j]+'" width="'+lons[j]+'" valida="'+valida[j]+'" onKey="'+onkey[j]+'"';
		caux+=' inicial="'+vini[j]+'" formula="'+formulas[j]+'" align="'+alins[j]+'" id="H_'+NomId+j+'" style="border:none" height="20" valor="'+valH[j]+'" dobleClick="'+dblclick[j]+'" htmldebase="'+htmldb[j]+'">';
		caux+="\n\t\t\t"+cab[j];
		caux+='\n\t\t</td>';
	}
	...
	if(paginador == 'S' || paginador == 's')
	{
		...
		caux+='<tr height="25" style="border:none" ><td class="mostrar_paginador_GridRC">';		
		...
		caux+='&nbsp;&nbsp;<img width="18" height="12" src="'+ruta+'first.jpg" onmouseover="this.style.cursor=\'hand\'; this.style.cursor=\'pointer\';" onclick="firstPaginador(\''+NomId+'\')">&nbsp;';
		caux+='<img width="18" height="12" src="'+ruta+'prev.jpg" onmouseover="this.style.cursor=\'hand\'; this.style.cursor=\'pointer\';" onclick="antPaginador(\''+NomId+'\')">&nbsp;&nbsp;';
		...
		caux+='&nbsp;&nbsp;<img width="18" height="12" src="'+ruta+'next.jpg" onmouseover="this.style.cursor=\'hand\'; this.style.cursor=\'pointer\';" onclick="sigPaginador(\''+NomId+'\')">&nbsp;';
		caux+='<img width="18" height="12" src="'+ruta+'last.jpg" onmouseover="this.style.cursor=\'hand\'; this.style.cursor=\'pointer\';" onclick="lastPaginador(\''+NomId+'\')">&nbsp;&nbsp;';
...
{	
	...
	switch(tipo)
	{
		...
		case 'fecha':			
			...
			break;
		case 'binario':
			if((obj.checked == true && celda.valor == 0) || (obj.checked == false && celda.valor == 1))
			{
				nver=true;
	...
	{			
		...
		{			
			...
			if (objx.offsetParent)
			{
				...
				{
					curleft += objx.style.position.left;
					curtop += objx.style.position.top;
				}
			...
			aux+='onfocus="document.getElementById(\''+tabla+'\').elefoc=\'com'+cad+'\';document.getElementById(\''+tabla+'\').setAttribute(\'elefoc\', \'com'+cad+'\');"';
...
function EliminaFila(cad)
{
	...
	var posElimina=Gral.posElimina?Gral.posElimina:Gral.getAttribute("posElimina");
...
function RecargaGrid(tabla, filar)
{
		...
		for(k=inicont;k<limMos;k++)
		{
			...
			for(l=0;l<axF.length;l++)
			{
				...
				if(tipos[l] == 'binario')//MS
				{
					...
					NewTabla+='<input type="checkbox" value="SI" '+sel+' onBlur="DesEditaCelda(\''+idcelda+'\', this);" id="c'+idcelda+'" onkeydown="TeclasEspeciales(event, this, \''+idcelda+'\');"  class="check" '+extra+'>';
				}
				else if(tipos[l] == 'libre')//MS
				{							
					if(htmldb[l]!="S")
	...
	if(paginador == 'S' || paginador == 's')
	{
		...
		/*caux+='&nbsp;<img alto="20" height="20" src="'+ruta+'first.jpg" onmouseover="this.style.cursor=\'hand\'; this.style.cursor=\'pointer\';">&nbsp;';
		caux+='<img alto="20" height="20" src="'+ruta+'prev.jpg" onmouseover="this.style.cursor=\'hand\'; this.style.cursor=\'pointer\';">&nbsp;';
		...
		caux+='<span id="numpages_paginador_'+NomId+'">1</span>';
		caux+='<td class="next_paginador_GridRC">';
		caux+='&nbsp;<img alto="20" height="20" src="'+ruta+'next.jpg" onmouseover="this.style.cursor=\'hand\'; this.style.cursor=\'pointer\';">&nbsp;';
		caux+='<img alto="20" height="20" src="'+ruta+'last.jpg" onmouseover="this.style.cursor=\'hand\'; this.style.cursor=\'pointer\';">&nbsp;';
...
function valorXYNoOnChange(tabla,cx,cy,valor)
{
	...
	if(!objC)
		return false;
	var onchange=objH.getAttribute("onchange");
