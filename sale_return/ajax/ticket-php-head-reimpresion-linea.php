...
//	}
	...
	// Constructor
		function TicketPDF1($orientation='P', $unit='mm', $size, $sucursal='', $pedido='', $inicio=10) {
	...
	$ticket = new TicketPDF1("P", "mm", array(80,130+$lineas_productos*6+($total!=$subtotal?12:0)+($pagado>0?14:30)+(count($pagos)>0?($lineas_pagos+1)*6:0)+30+10), "{$sucursal}", "{$folio}", 10);
	$ticket->AliasNbPages();
	$ticket->AddPage();
...
//AQUI ENTRA DETALLE DE DESCUENTOS
	...
	if (count($pagos)) {
		...
		$ticket->SetXY(4+66*0.4, $ticket->GetY()+5);
		$ticket->Cell(66*0.3, 6, utf8_decode("Total"), "" ,0, "L");
			
		$ticket->SetXY(5+66*0.75, $ticket->GetY());
		$ticket->Cell(66*0.25, 6, "$ " . number_format($total, 2), "" ,0, "R");//_pagos
